...
const Home = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/users.json/${props.sessionId}`)
      .then((resp) => {
  ...
  const deleteHandler = (userIndex) => {
    axios
      .delete(
        `http://localhost:5000/users.json/${props.sessionId}?id=${userIndex}`
      )
      .then((resp) => {
        if (typeof resp.data === "object") {
          setUserData(resp.data);
    ...
    <div className="Home">
      {!props.githubData ? (
        ...
        <React.Fragment>
          ...
          <div>Followers: {props.githubData.followers}</div>
        </React.Fragment>
      )}
